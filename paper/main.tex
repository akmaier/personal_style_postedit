...
\else
  % Real authors and institution. Used by `make arxiv` to build
  % `main_arxiv.pdf`. Order: project lead first, advisory co-author second.
  \author{Andreas K. Maier\inst{1} \and Siming Bayer\inst{1}}
  \authorrunning{A.\,K. Maier and S. Bayer}
...
(length-only AUC $=$ $0.517$, full LUAR AUC $=$ $0.952$). Closing
the loop, we let an Opus\,4.7 agent see the frozen detector's score
and rewrite a held-out draft for $T{=}20$ iterations: the agent
flips two of five test targets to the human half-space and shrinks
the margin on every target by an order of magnitude. The arms race
is real but its shape is asymmetric, and the strongest LLM agents
already partially defeat the strongest LUAR detector. All code,
all 648 mimic drafts, the trained detectors, every diagnostic,
the adversarial trajectories, and the agentic harness are released.
