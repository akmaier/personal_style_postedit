...
of LLM drafts---and add three new ones---in roughly three hours of
wall-clock work, with the human investigator acting only as a
reviewer-in-the-loop. We reproduce all seven preregistered
hypotheses and recover the paper's headline rmcorr to three decimal
places ($r{=}{+}0.244$, $p{<}10^{-8}$, $n{=}648$). Under a
leakage-free held-out protocol, GPT-5.5 and Claude\,Opus\,4.7 close
$71$--$75\,\%$ of the style gap to the same-author ceiling on $324$
paired tasks, against $24\,\%$ for the human post-edit, and beat the
human on $\sim$$80\,\%$ of tasks.
...
costs roughly three hours of wall-clock work, not weeks}. An
...
and re-ran the pipeline against. The wall-clock cost of one full
...
the source paper's full LUAR-MUD\,/\,permutation-test\,/\,rmcorr
pipeline from the released logs and reproduce every preregistered
hypothesis. Section~\ref{sec:extension} extends the design with a
...
$100$--$200$ word range. No author overlap between training pids
and target pids; \texttt{tests/test\_adversarial\_no\_leakage.py}
makes that load-bearing. Fig.~\ref{fig:adversarial} shows the
trajectories: every target's margin shrinks substantially (mean
final $-0.16$ vs.\ initial $+3.13$); two of the five
(\texttt{thank}, \texttt{letter}) cross the SVM decision boundary
into ``human'' and stay there; the other three approach zero but
do not stabilise on the human side by iteration $20$. The frozen
detector is \emph{partially} but not \emph{robustly} defeated by
an Opus 4.7 agent given $20$ iterations of feedback---the
strongest version of the arms race in this paper.
