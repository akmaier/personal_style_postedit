...
costs roughly three hours of wall-clock effort, not weeks}. We use
``wall-clock'' here in its standard sense---real elapsed time as it
would appear on a clock on the wall, including waits for compute and
LLM responses---rather than CPU-time or person-hours. The
...
and re-ran the pipeline against.
...
The closest setup in the literature on agentic LLM evaluation is
...
loop or light fine-tuning of the rewriting strategy is expected to
flip the rest. The current snapshot favours the agent.
