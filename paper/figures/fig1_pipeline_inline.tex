...
\begin{tikzpicture}[
  ...
  \node[proc, below=18pt of luar] (gen) {LLM mimic\\generation\\\texttt{07\_gen}};
  ...
  % Drafts box, placed to the left of `gen` (about a cm leftward of its
  % previous mid-position). It feeds into the LLM mimic generation step.
  ...
  % Branches into the extension lane. Both arrows now end at the WEST
  % anchor of an actual node so they don't point into empty space:
  %   tidy -> gen      (planning details flow into mimic generation)
  %   luar -> mimsim   (the same LUAR embedder also embeds the mimics)
  \draw[->] (tidy.south) |- (gen.west);
  \draw[->] (luar.south) |- (mimsim.west);
